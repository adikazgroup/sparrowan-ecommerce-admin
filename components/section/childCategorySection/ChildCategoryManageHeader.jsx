...
export default function ChildCategoryManageHeader({
  ...
  const { data: categoryData } = useGetCategoriesIdNameQuery({ level: 1 });
  const subCategoryOptions = [
    { value: "", label: "All Sub Categories" },
    ...(categoryData?.data?.map((c) => ({ value: c.value, label: c.label })) ||
      []),
  ];
      ...
      <div className="flex sm:flex-row flex-col sm:items-center justify-between gap-4">
        ...
        <Link href={addHref}>
          <Button className="whitespace-nowrap">
            <LuPlus className="size-4" /> Add Child Category
          </Button>
        </Link>
      </div>
