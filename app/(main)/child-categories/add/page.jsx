...
import { LuArrowLeft, LuSave, LuX, LuImage } from "react-icons/lu";
...
export default function AddChildCategoryPage() {
  ...
  const { data: subCategoryData } = useGetCategoriesIdNameQuery({ level: 1 });

  const departmentOptions =
    departmentData?.data?.map((d) => ({ value: d.value, label: d.label })) ||
    [];
  const subCategoryOptions =
    subCategoryData?.data?.map((c) => ({ value: c.value, label: c.label })) ||
    [];
  ...
  const validateForm = () => {
    ...
    if (!formData.parent) newErrors.parent = "Parent sub-category is required";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    const loadingToast = toast.loading("Creating child-category...");
    const result = await createCategory(generateFormData(payload));

    handleToast({
      result,
      type: result?.data ? "success" : "error",
      id: "create-childcategory",
      message: "Child-category created successfully!",
    });
    toast.dismiss(loadingToast);

    if (result?.data) router.push("/child-categories");
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Create a new child-category under a sub-category
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
          ...
          <Button type="submit" disabled={isLoading}>
            ...
            {isLoading ? "Creating..." : "Create Child-Category"}
          </Button>
        </div>
      </form>
    </div>
